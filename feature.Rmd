...
validation.NoNA<-validation[which(is.na(training$entertainment.y)==FALSE)]
...
```
## Remove multiollinearity

Removed:

- Weekend (correlated with Weekday)
- Morning (correlated with Afternoon)
...
# Delete one of morning/afternoon
  select(-one_of("morning")) %>%
# Delete one date
  select(-one_of("2016-05-01")) %>%
...
```{r}
training.NoNA.NoMultc <- fread( "training_NoNA_NoMultc.csv")
validation.NoNA.NoMultc <- fread( "validation_NoNA_NoMultc.csv")
knn_3 <- kNN(training.NoNA.NoMultc,variable=c("longitude","latitude"),k=3) #k=3
```
```{r}
...
```{r}
training <- fread("training.csv", header = T, integer64 = "character")
count.train <- subset(training, select = beauty.x:vehicle.x)
ratio.train <- subset(training, select = beauty.y:vehicle.y)
count.train <- na.omit(count.train)  # 18629    25
ratio.train <- na.omit(ratio.train)  # 18594    24
```

```{r}
fwrite(count.train, 'count_train.csv')
fwrite(ratio.train, 'ratio_train.csv')
```

```{r}
count.train <- fread("count_train.csv", header = T, integer64 = "character")
ratio.train <- fread("ratio_train.csv", header = T, integer64 = "character")
```

```{r}
# conduct PCA on the two datasets
count.pca <- prcomp(count.train[,-1], center = TRUE, scale. = TRUE)
summary(count.pca)   # 11 PCs for cumulative proportion of >90% 
ratio.pca <- prcomp(ratio.train[,-1], center = TRUE, scale. = TRUE) 
summary(ratio.pca)   # 15 PCs for cumulative proportion of >90% 

count.train.pca <- data.frame(device_id = count.train$device_id, count.pca$x[,1:11])
ratio.train.pca <- data.frame(device_id = ratio.train$device_id, ratio.pca$x[,1:15])
```
...
# test 
validation <- fread("validation.csv", header = T, integer64 = "character")
count.test <- subset(validation, select = beauty.x:vehicle.x)
ratio.test <- subset(validation, select = beauty.y:vehicle.y)
count.test <- na.omit(count.test)  # 4661    25
ratio.test <- na.omit(ratio.test)  # 4653    24
```

```{r}
fwrite(count.test, 'count_test.csv')
fwrite(ratio.test, 'ratio_test.csv')
```

```{r}
count.test.pca <- predict(count.pca, newdata = count.test[,-1])
count.test.pca <- data.frame(device_id = count.test$device_id, count.test.pca[,1:11])
ratio.test.pca <- predict(ratio.pca, newdata = ratio.test[,-1])
ratio.test.pca <- data.frame(device_id = ratio.test$device_id, ratio.test.pca[,1:15])
```
