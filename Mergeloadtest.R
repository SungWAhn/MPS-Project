...
device <- phonebrand$device_model
...
phonebrand$device_model <- device
...
training <- merge(x = merged2, y = app_events, by = 'event_id')
...
finalmerge <- merge(x = setDT(training), y = setDT(labelsdat), by = 'app_id', allow.cartesian = T)
