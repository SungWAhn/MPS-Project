...
leftoversmod <- phonebrand[which(grepl('[A-Za-z]', '\\d', phonebrand$phone_brand) == FALSE), ]
leftoversmod
```
...
```{r }
ggplot(data=train[is.na(phone_brand)==0],aes(x=phone_brand,fill=phone_brand))+
  geom_bar(stat="count")+
  ggtitle("Phone Brand Distribution")
...
ggplot(data = phonebrandsorted, aes(x = reorder(phone_brand, -count), y = count)) +
