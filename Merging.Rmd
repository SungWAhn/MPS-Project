...
**Main Points:**
Before merging the datasets, determine which columns are unique and which have duplicates.

- events: **Unique** event_id and nonunique device_id. I.e. Every event_id is unique, but multiple events_ids could have same device_id.
- App_events: App_id and event_id not unique. I.e. Single event_id can have multiple app_ids associated. Multiple event_ids can have same app_ids.
- app_labels: App_id and label_id not unique. I.e. App_ids can be categorized under multiple label_ids, and the same label_ids are used multiple times for different app_ids
...
kable(app_events[app_id == '6284164581582112235',][1:10], caption = 'Same Apps are used in Multiple Events') %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "responsive"), full_width = F)
```
For every event_id, there are multiple app_ids associated. Same app_ids can be associated with different event_id. This dataset has no unique index.
...
```{r}
merged1a <- merge(x = gender_age, y = events, by = 'device_id', all.x = T)
merged2a <- merge(x = merged1a, y = app_events, by = 'event_id', all.x = T)
traina <- merge(x = merged2a, y = setDT(uniquephones), by = 'device_id', all.x = T)
traina.noNA <- merged2a[is.na(traina$phone_brand)==FALSE | is.na(traina$event_id)==FALSE]
```
...
labeltrain2 <- arrange(labeltrain2, event_id)
```