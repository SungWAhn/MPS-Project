...
- Count of how many unique events occurred in the AM or in the PM (2 predictors total)
- Count for how many unique events a device_id had between April 30 - May 8 the event occurred (9 predictors total)
- Count for how many unique events a device_id had on weekdays and on weekends (2 predictors total)
- Count of number of events total (1 predictor total)
- Count of how many unique apps a device_id used in each app category (24 predictors total, for each category)
...
- Ratio of the number of is_active to number of unique apps for each app category (23 predictors total)
- Response variable group (1 column)
...
```{r}
train <- fread("train.csv", header = T, integer64 = "character")
setDT(train[,-1])
labelsdat <- fread("labelsdat.csv", header = T, integer64 = "character")
labeltrain <- fread("labeltrain.csv", header = T, integer64 = "character")
gender_age <- fread("gender_age_train.csv", header = T, integer64 = "character")
```

Is active percent for each 
```{r}
