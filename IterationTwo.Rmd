...
### Performance
```{r results='asis'}
performancecateg <-data.frame('features' = c('Count features','Count features','Count features','Count features'), 'Model' = c('NB','RF','SVM','Logistic'), 'Logloss' = c(21.06245,2.264044,2.374288,2.374288), 'Misclass' = c(0.9183323,0.7964754,0.8214055,0.8132388))
...
candidates <- NULL
for (i in 1:63) {
 if(length(corrM[which(corrM[,i]!=0),i]) > 10) {
   candidates[i]<-(names(corrM[,i])[i])
 }
}
candidates<-candidates[which(is.na(candidates)==FALSE)]
```
...
```{r}
traincontuncorr <- select(traintotal, -c(highcorr[1:length(highcorr)]))
validcontuncorr <- select(validtotal, -c(highcorr[1:length(highcorr)]))
traincontuncorr$group<- as.factor(traincontuncorr$group)
validcontuncorr$group<-as.factor(validcontuncorr$group)
```
```{r}
fwrite(traincontuncorr, 'traintotaluncorr.csv')
fwrite(validcontuncorr, 'validtotaluncorr.csv')
```
...
```
```{r}
kable(data.frame('Misclassification'=paste0((100*round(misclassnb,4)),'%'), 'logloss' = loglossnb), caption = 'Naive Bayes Performance: Without Correlated Predictors') %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "responsive"), full_width = F)
```
```{r fig.width=6, fig.height=5.5}
confusion <- as.data.frame(table(prednb, validcontuncorr$group))
p<-ggplot(confusion, aes(Var2, prednb)) +
  geom_tile(aes(fill = Freq), colour='gray') +
  scale_fill_gradient(low='antiquewhite', high = 'purple') +
  theme(axis.text.x = element_text(angle = 80, hjust = 1, size = 9),
        axis.text.y = element_text(size = 9),
...
ggplot(data = train.final, aes(x = count_event)) +
...
ggplot(data = train.final, aes(x = count.app)) +
  geom_histogram(binwidth = 10) +
  #scale_fill_manual(values=c("#F44336", "#E91E63", "#9C27B0", "#673AB7", "#3F51B5", "#2196F3", "#03A9F4", "#00BCD4", "#009688", "#4CAF50", "#8BC34A", "#CDDC39")) +
  xlim(c(0,250)) +
  ggtitle("App Count")+
...
# health, photography, event count, app count
...
## Hourly Counts
```{r}
count48<-data.matrix(train.NoNANoM[, 'count.4_8'])
quantile48<-quantile(count48, c(0.25, 0.5, 0.75)) # 0 0 5
count48[count48==0]<-0
count48[count48>0 & count48<5]<-1
count48[count48>=5]<-2

count812<-data.matrix(train.NoNANoM[, 'count.8_12'])
quantile812<-quantile(count812, c(0.25, 0.5, 0.75)) # 0   2  10 
count812[count812>=0 & count812<2]<-0
count812[count812>=2 & count812<10]<-1
count812[count812>=10]<-2

count1216<-data.matrix(train.NoNANoM[, 'count.12_16'])
quantile1216<-quantile(count1216, c(0.25, 0.5, 0.75)) # 0   2  9
count1216[count1216>=0 & count1216<2]<-0
count1216[count1216>=2 & count1216<9]<-1
count1216[count1216>=9]<-2

count1620<-data.matrix(train.NoNANoM[, 'count.16_20'])
quantile1620<-quantile(count1620, c(0.25, 0.5, 0.75)) # 0   2  9
count1620[count1620>=0 & count1620<2]<-0
count1620[count1620>=2 & count1620<9]<-1
count1620[count1620>=9]<-2

count2024<-data.matrix(train.NoNANoM[, 'count.20_24'])
quantile2024<-quantile(count2024, c(0.25, 0.5, 0.75)) # 0   2  10
count2024[count2024>=0 & count2024<2]<-0
count2024[count2024>=2 & count2024<10]<-1
count2024[count2024>=10]<-2
...
count812<-data.matrix(valid.NoNANoM[, 'count.8_12'])
quantile812<-quantile(count812, c(0.25, 0.5, 0.75)) # 0   2  10 
count812[count812>=0 & count812<2]<-0
count812[count812>=2 & count812<10]<-1
count812[count812>=10]<-2

count1216<-data.matrix(valid.NoNANoM[, 'count.12_16'])
quantile1216<-quantile(count1216, c(0.25, 0.5, 0.75)) # 0   2  9
count1216[count1216>=0 & count1216<2]<-0
count1216[count1216>=2 & count1216<9]<-1
count1216[count1216>=9]<-2

count1620<-data.matrix(valid.NoNANoM[, 'count.16_20'])
quantile1620<-quantile(count1620, c(0.25, 0.5, 0.75)) # 0   2  9
count1620[count1620>=0 & count1620<2]<-0
count1620[count1620>=2 & count1620<9]<-1
count1620[count1620>=9]<-2

count2024<-data.matrix(valid.NoNANoM[, 'count.20_24'])
quantile2024<-quantile(count2024, c(0.25, 0.5, 0.75)) # 0   2  10
count2024[count2024>=0 & count2024<2]<-0
count2024[count2024>=2 & count2024<10]<-1
count2024[count2024>=10]<-2
...
training$group <- as.factor(training$group)
validation$group <- as.factor(validation$group)
traincateg<-cbind(training, trainingIsactive, trainingWeekday, traincount, train.daycat)
validcateg<-cbind(validation, validIsactive, validWeekday, validcount, valid.daycat)
...
```{r results='asis'}
performancecateg3 <-data.frame('features' = c('Is_Active & App Ct Categ','Is_Active & App Ct Categ','Is_Active & App Ct Categ','Is_Active & App Ct Categ'), 'Model' = c('NB','RF','SVM','Logistic'), 'Logloss' = c(2.713002,2.422743,2.309568,2.315649), 'Misclass' = c(0.8396733,0.8334408,0.8280679,0.8166774))
kable(performancecateg3, caption = 'Performance of Is_active & App Ct & Date Ct Categorical Variables') %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "responsive", "condensed"), full_width = F)
```

```{r results='asis'}
performancecateg4 <-data.frame('features' = c('Is_Active & App Ct Categ','Is_Active & App Ct Categ','Is_Active & App Ct Categ','Is_Active & App Ct Categ'), 'Model' = c('NB','RF','SVM','Logistic'), 'Logloss' = c(2.713002,2.422743,2.309568,2.315649), 'Misclass' = c(0.8396733,0.8334408,0.8280679,0.8166774))
kable(performancecateg4, caption = 'Performance of Is_active & App Ct & Date Ct &  Categorical Variables') %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "responsive", "condensed"), full_width = F)
```
