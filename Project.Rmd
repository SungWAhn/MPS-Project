...
# Convert to int64 since double loses precision
...
Missing values in longitude/latitude
```{r}
...
```{r}
# Count number of events each group category has
...
kable(escape = F, caption = 'Event Count by Group') %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "responsive"), full_width = F)
```
...
```{r results='asis', echo=FALSE}
kable(phone_count, caption = 'Phone Devices By Group') %>%
