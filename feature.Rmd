...
labeltrain <- labeltrain[order(device_id)]

sum.isactive.bycat <- labeltrain %>%
  filter(is_active == 1) %>%
  group_by(device_id, category, is_active) %>%
  dplyr::summarise(count = n())

sum.apps.bycat <- labeltrain %>%
  filter(is_active == 1) %>%
...
for (i in 1:length(deviceid)) {
  if(sum.isactive.bycat$device_id[i] == deviceid[i] & sum.isactive.bycat$category == 'business') {
    feature[i]
  }
}
