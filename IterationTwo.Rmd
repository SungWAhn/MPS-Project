...
## Check the conditional distributions: How varied are the Groups?
```{r}
...
predictions1 <- cbind(predictions1, valid1vars)
```
```{r}
...
```{r}
ggplot(data=train.NoNANoM, aes_string(x='group', y='count.app'))+
  geom_jitter(position = position_jitter(.2), na.rm = TRUE) +
  ylim(c(0,100))
```
If we zoom in onto a smaller subsection of app count, there does seem to be some variation. In the female groups, there are generally more smaller numbers, while the male groups generally seem to have higher count numbers. Naive Bayes however doesn't seem to pick this up. Maybe engineer a different feature that can pick this up.
```{r}
#features: count.app, count_event, weekday, group
traince<-train.final[,-c(1,6,29:44,7,16)]
validce<-valid.final[,-c(1,6,29:44,7,16)]
fitce<-naiveBayes(as.factor(group)~., data=traince)
validce$group<-as.factor(validce$group)
predce<-predict(fitce, validce)
summary(predce)
# F23- F24-26 F27-28 F29-32 F33-42   F43+   M22- M23-26 M27-28 M29-31 M32-38   M39+ 
#1566      0      0      0      0      0   2136    283      9      0    428    231 
mean(predce!=validce$group)
#[1] 0.9024285
```
```{r}
confusionce <- as.data.frame(table(predce, validce$group))
p<-ggplot(confusionce, aes(Var2, predce)) +
  geom_tile(aes(fill = Freq), colour='gray') +
  scale_fill_gradient(low='antiquewhite', high = 'purple') +
  theme(axis.text.x = element_text(angle = 80, hjust = 1, size = 9),
        axis.text.y = element_text(size = 9),
