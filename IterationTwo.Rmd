...
```{r}
traintotal <- train.NoNANoM[,-c(1,66,67)]
validtotal <- valid.NoNANoM[,-c(1,66,67)]
...
summary(predcateg)
mean(predcateg!=validcateg$group)
...
### Performance
```{r results='asis'}
performancecateg <-data.frame('features' = c('All categorical features','All categorical features','All categorical features','All categorical features'), 'Model' = c('NB','RF','SVM','Logistic'), 'Logloss' = c(2.711981,2.499703,2.307586,2.300363), 'Misclass' = c( 0.8441865,0.8295723,0.8269933,0.815173))
kable(performancecateg, caption = 'Performance of Categorical Variables') %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "responsive", "condensed"), full_width = F)
```
...
traingender <- traincateg
traingender$group <- gender

group <- validcateg$group
gender <- substr(group, 1, 1)
validgender <- validcateg
validgender$group <- gender


traingender$group <- as.factor(traingender$group)
validgender$group <- as.factor(validgender$group)
names(traingender)[18] <- 'socialnetworking.appCt'
names(validgender)[18] <- 'socialnetworking.appCt'
fitgender<-naiveBayes(group~., data=traingender)
predgender<-predict(fitgender, validgender)
summary(predgender)
mean(predgender!=validgender$group)

#log-loss
predgenderprobs <-predict(fitgender, validgender, type = 'raw')
predgenderprobsdat <- data.frame(validgender$group, predgenderprobs)
logloss <- vector(length = length(predgenderprobsdat$validgender.group))
for (i in 1:length(predgenderprobsdat$validgender.group)) {
  clas=grep(str_split(paste(predgenderprobsdat$validgender.group[i]), '-')[[1]][1], names(predgenderprobsdat))
  logloss[i] = log(predgenderprobsdat[i,clas])
}
-mean(logloss)
```
